...
fxvolume.addEventListener("change", function (f) {
  buysound.volume = f.currentTarget.value / 100;
});

fxvolume.addEventListener("change", function (g) {
  buttonfx.volume = g.currentTarget.value / 100;
});

fxvolume.addEventListener("change", function (h) {
  mainbuttonfx.volume = h.currentTarget.value / 100;
});
